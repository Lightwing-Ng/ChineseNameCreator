...
class ChineseName(object):
    ...
    def __init__(self, firstname_file=None, lastname_file=None):
        """
        初始化
        :param firstname_file: 文件路径
        :param lastname_file:  文件路径
        """
    ...
    def getName(self, char_count=1, lastname=''):
        """
        获取1个中文名字
        :param char_count: 名字长度，默认1 - Integer
        :param lastname: 姓氏，默认随机
        :return: 名字
        """
        char_count = random.randint(1, 3)
        name = []

        if char_count <= 3 and char_count >= 1:
            if lastname == '':
    ...
    def getNames(self, count, char_count=1, lastname=''):
        """
        获取一个中文名列表，利用getName
        :param count: 名字数量 - Integer
        :param char_count: 名字长度，默认1 - Integer
        :param lastname: 姓氏，默认随机 - String
        :return: 名字列表
        """
        names = []
...
def main():
    demo = ChineseName()  # 实例化

    # 按姓名定制
    SurName = str(input('姓氏: '))
    N = int(input('个数: '))
    for x in range(N):
        name = demo.getName(lastname=SurName)
        print(name, ' ')

    # 获取一个姓名生成器
    N = int(input('\n请输入随机生成中文名的数量(<100)：'))
    name_generator = demo.getNameGenerator(N)
    if N < 100:
        for x in name_generator:
            print(x, end=' ')
    else:
        print("Sorry!")
    print('\n')
